  ...
  return(
    <div className={`w-full lg:mb-0 bg-white hover:shadow-md rounded-2xl border border-${theme.color}-50`}>
      ...
      <div className="p-3">
        ...
        <p className="text-gray-400 px-3 flex text-xs justify-center mb-5"><MapPinIcon className="w-3 h-3 mt-0.5 mr-1"/> {breeder.address}</p>
        <div className="flex justify-center items-center">
          <Typography
            as="a"
            href={`/breeder/${breeder.id}/follow`}
            className={`p-2 px-5 text-center bg-green-50 text-green-800 font-semibold rounded-full mt-5 mb-3 w-full hover:bg-green-800 hover:text-white`}
          >
            Hire Me
          </Typography>
          {/* <Typography
